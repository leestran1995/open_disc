...
create sequence open_discord.server_event_seq
    as integer;

alter sequence open_discord.server_event_seq owner to appuser;
...
create table if not exists open_discord.server_events
(
    id          uuid                     default gen_random_uuid() not null
        constraint server_events_pk
            primary key,
    event_type  varchar(128)                                       not null,
    payload     json,
    timestamp   timestamp with time zone default CURRENT_TIMESTAMP not null,
    event_order integer                  default nextval('open_discord.rooms_default_order'::regclass)
);

alter table open_discord.server_events
    owner to appuser;

create index if not exists server_events_timestamp_index
    on open_discord.server_events (timestamp desc);
